...
def get_all_latents(img_np, models, inv_seed=1):
    """Gets latent representations of an image at different noise levels for Stable Diffusion.
    
    This function generates a sequence of latent representations by:
    1. Encoding the input image into the initial latent space using the VAE encoder
    2. Creating text embeddings for a generic "realistic photo" conditioning prompt
    3. Progressively adding Gaussian noise to the latents through the diffusion process
       to generate a sequence of increasingly noisy latent states
    
    The noise schedule follows Stable Diffusion's forward diffusion process, starting 
    from the clean image latents and gradually adding more noise at each step until
    reaching pure noise. This creates a sequence of latent representations that capture
    the image at different levels of abstraction.
    
    Args:
        img_np: Input image as a numpy array (H,W,C) in RGB format
        models: Model dictionary containing VAE, CLIP tokenizer, text encoder etc.
        inv_seed: Random seed for reproducible noise generation (default: 1)
    
    Returns:
        all_latents: Tensor of shape (num_steps, 1, 4, H/8, W/8) containing the latent 
                    representations at each noise level, from clean to noisy
        input_embeddings: CLIP text embeddings for the conditioning prompt
    """
    # Set random seed for reproducible noise generation
    generator = torch.cuda.manual_seed(inv_seed)
    
    # Encode input image into initial clean latent representation using VAE
    cln_latents = pipelines.encode(models.model_dict, img_np, generator)
    
    # Create text embeddings for a generic scene prompt to condition the diffusion
    # Note: Using specific background prompts from LLM was tried but generic works better
    ...
    # Generate sequence of noisy latents by running forward diffusion
    # Uses 50 noise steps with guidance_scale=2.5 to balance detail and stability
