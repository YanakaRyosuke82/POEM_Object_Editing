...
def create_folder_structure():
    """
    This script serves to create a small experiment fron a list of input images.
    It takes a collection of source images from prompt folders and organizes them
    into a structured experiment directory with consistent inputs, masks, and edit
    instructions for each sample.

    Specifically, this script:
    1. Creates a main experiment directory ('exp_teaser_figure_auto_2') with an 'input' subdirectory
    2. Finds all images in prompt folders from the 'generated_images_2' source directory
    3. For each image:
       - Creates a numbered sample folder (sample_001, sample_002, etc.)
       - Copies the original image as 'input_image.png'
       - Creates a dummy mask with a square in the center (1/3 of image size) to define the edit region
       - Adds a randomly selected edit instruction from predefined options

    The resulting dataset structure follows a standardized format that can be processed
    by image editing model evaluation pipelines, allowing for consistent comparison
    of different editing techniques across the same inputs.
    """
    # Create main folder
