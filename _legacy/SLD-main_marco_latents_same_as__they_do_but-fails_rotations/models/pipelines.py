...
from utils_pose import guidance, schedule, boxdiff
import utils_pose
...
def generate_semantic_guidance(
    ...
    for index, t in enumerate(tqdm(scheduler.timesteps, disable=not show_progress)):
        ...
        if bboxes:
            if use_boxdiff:
                latents, loss = boxdiff.latent_backward_guidance_boxdiff(
                    scheduler,
                    unet,
                    cond_embeddings,
                    index,
    ...
    if return_box_vis:
        pil_images = [
            utils_pose.draw_box(Image.fromarray(image), bboxes, phrases) for image in images
        ]
...
def generate_gligen(
    ...
    if return_box_vis:
        pil_images = [
            utils_pose.draw_box(Image.fromarray(image), bboxes_item, phrases_item)
    ...
    for index, t in enumerate(tqdm(scheduler.timesteps)):
        ...
        with torch.no_grad():
            ...
            if index < frozen_steps:
                latents = latents_all[index + 1] * frozen_mask + latents * (
                    1.0 - frozen_mask
                )
...
def generate_partial_frozen_new(
    model_dict,
    latents_all,
    frozen_mask,
    input_embeddings,
    ...
    latents = latents_all[0].cuda() * frozen_mask + latents * (1.0 - frozen_mask)
...
def generate_partial_frozen_remove(
    model_dict,
    latents_all,
    frozen_mask,
    input_embeddings,
    ...
    for index, t in enumerate(tqdm(scheduler.timesteps)):
        if index < frozen_steps:
            latents = latents_all[index].cuda() * frozen_mask + latents * (
                1.0 - frozen_mask
            )
        with torch.no_grad():
            # expand the latents if we are doing classifier-free guidance to avoid doing two forward passes.
...
def generate_partial_frozen_remove_new(
    model_dict,
    latents_all,
    frozen_mask,
    input_embeddings,
    ...
    for index, t in enumerate(tqdm(scheduler.timesteps)):
        with torch.no_grad():
            ...
            if index < frozen_steps:
                latents = latents_all[index + 1].cuda() * frozen_mask + latents * (
                    1.0 - frozen_mask
                )
    # scale and decode the image latents with vae
    scaled_latents = 1 / 0.18215 * latents
