...
def run_sam_refine(file_analysis_path: str, img_path: str, sam_model: Any):
    ...
    with open(enhanced_file, 'w') as f:
        ...
        # Extract content using regex and only write once
        scene_desc = re.search(r'Scene Description:\n(.*?)\n', content)
        spatial_rel = re.search(r'Spatial Relationships:\n(.*?)(?=\n\n|\n[A-Z]|$)', content, re.DOTALL)
        background_desc = re.search(r'Background Description:\n(.*?)(?=\n\n|\n[A-Z]|$)', content, re.DOTALL)
        generation_prompt = re.search(r'Generation Prompt:\n(.*?)(?=\n\n|\n[A-Z]|$)', content, re.DOTALL)
        
        f.write(f"\nScene Description:\n{scene_desc.group(1) if scene_desc else ''}\n")
        f.write(f"\nSpatial Relationships:\n{spatial_rel.group(1).strip() if spatial_rel else ''}\n")
        f.write(f"\nBackground Description:\n{background_desc.group(1).strip() if background_desc else ''}\n")
        f.write(f"\nGeneration Prompt:\n{generation_prompt.group(1).strip() if generation_prompt else ''}\n")
        f.write("\n=====================\n")
